...
fun codegen frame stm = 
	let val ilist = ref ([]:(instr list)) (*lista de instrucciones que va a ir mutando*)
        ...
        fun munchExp (CONST i) = result (fn r => emit(OPER{assem = "mov $"^(toString i)^", %'d0", src = [], dst = [r], jump = NONE}))
        | munchExp (NAME lab) = result (fn r => emit(OPER{assem = "mov $"^(makeString lab)^", %'d0", src = [], dst = [r], jump = NONE})) (*se podria retornar lab directo pero por las dudas*)
        | munchExp (MEM m) = result (fn r => emit(OPER{assem = "mov %'s0, %'d0", src =[munchExp m] , dst=[r], jump=NONE}))
        | munchExp (TEMP t) = t
        ...
        | munchExp (BINOP (PLUS, CONST i, e1)) = result ( fn r => (emit(MOV{assem = "mov %'s0, %'d0", src = munchExp e1, dst=r}); emit(OPER{assem = "add %'d0, $"^(toString i), src = [r], dst = [r], jump = NONE})))
        | munchExp (BINOP (PLUS, e1, CONST i)) = munchExp (BINOP (PLUS, CONST i, e1))
        | munchExp (BINOP (PLUS, e1, e2)) = result ( fn r => (emit(MOV{assem = "mov %'s0, %'d0", src=munchExp e1, dst=r}); emit(OPER{assem = "add %'d0, %'s1", src = [r, munchExp e2], dst = [r], jump = NONE})))
        | munchExp (BINOP (MINUS, CONST i, e1)) = result ( fn r => (emit(OPER{assem = "mov $"^(toString i)^", %'d0", src = [], dst=[r], jump=NONE}); emit(OPER{assem = "sub %'s1, %'d0", src = [r, munchExp e1], dst = [r], jump = NONE})))
        | munchExp (BINOP (MINUS, e1, CONST i)) = result ( fn r => (emit(MOV{assem = "mov %'s0, %'d0", src = munchExp e1, dst=r}); emit(OPER{assem = "sub %'d0, $"^(toString i), src = [r], dst = [r], jump = NONE})))
        | munchExp (BINOP (MINUS, e1, e2)) = result ( fn r => (emit(MOV{assem = "mov %'s0, %'d0", src=munchExp e1, dst=r}); emit(OPER{assem = "sub %'d0, %'s1", src = [r, munchExp e2], dst = [r], jump = NONE})))
        ...
        | munchExp (BINOP (DIV, CONST i, e1)) = result ( fn r => (
			let val m1 = munchExp e1
				val _ = emit(OPER{assem = "xor %'s0, %'d0", src=["edx"], dst=["edx"], jump = NONE})
				val _ = emit(OPER{assem = "mov $"^(toString i)^", %'d0", src=[], dst=["eax"], jump = NONE})
				val _ = emit(OPER{assem = "idiv %'s1", src = ["eax", "edx", m1], dst = ["eax", "edx"], jump = NONE})
				val _ = emit(MOV{assem = "mov %'s0, %'d0", src="eax", dst=r})
			in
				()
			end))
        | munchExp (BINOP (DIV, e1, CONST i)) = result ( fn r => (
			let val m1 = munchExp e1
				val _ = emit(OPER{assem = "xor %'s0, %'d0", src=["edx"], dst=["edx"], jump = NONE})
				val _ = emit(OPER{assem = "mov %'s0, %'d0", src=[m1], dst=["eax"], jump = NONE})
	    ...
	    | munchStm (MOVE(MEM e1, e2)) = emit (MOV{assem="mov %'s0, %'d0", src=munchExp e2, dst=munchExp e1})
	    | munchStm (MOVE(TEMP l, e))  = emit (MOV{assem="mov %'s0, %'d0", src=munchExp e, dst=l})
...
(*ojo que tal vez el cmp tiene los argumentos al reves*)
        ...
        | munchStm (EXP (CALL (NAME lab,args))) =
            ...
            in
				if spoffset>0
					then emit(OPER{assem = "add $"^(toString spoffset)^", 'd0", src = [tigerframe.sp], dst = [tigerframe.sp], jump = NONE})
					else ()
				end
